...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb"> <a href="{{ url('admin/dashboard') }}" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#">Approval Products</a> <a href="#" class="current">View Products</a> </div>
    <h1>Products to be Approve Section</h1>
            ...
            <table class="table table-bordered data-table">
              ...
              <tbody>
                ...
                <tr class="gradeX">
                  ...
                  <td style="text-align:center;"><span style="display: none;">{{ $loop->index+1 }}</span>{{ $product->id }}</td>
                  ...
                  <td style="display: none;">@if($product->featured==1)<span style="color: green; font-weight: bold;">featured</span>@else<span style="color: red; font-weight: bold;">No</span>@endif</td>                  
                  <td>
                  ...
                  <td style="text-align: center;">
                    <form action="{{ url('admin/product-approved/'.$product->id) }}" method="post">
                        {{ csrf_field() }}
                        <input type="checkbox" class="custom-control-input" name="admin_approved" @if($product->admin_approved=="1") checked @endif value="1" onchange="javascript:this.form.submit();">
                    </form>
                  </td>

                  <td class="center">
                    <a href="#myModal{{ $product->id }}" data-toggle="modal" class="btn btn-success" title="View Product"><i class="fa fa-eye"></i></a>
                    <a href="{{ url('/admin/edit-product/'.$product->id) }}" class="btn btn-primary" title="Edit Product "><i class="fa fa-pencil-square-o"></i></a>
                    <a href="{{ url('/admin/add-attributes/'.$product->id) }}" class="btn btn-info" title="Add Attributes"><i class="fa fa-list"></i></a>
                    <a href="{{ url('/admin/add-images/'.$product->id) }}" class="btn btn-warning" title="Add Images"><i class="fa fa-picture-o"></i></a>
                    <a onclick="return confirm('Are you sure you want to delete this Product?');" href="{{ url('/admin/delete-product/'.$product->id) }}" class="btn btn-danger" title="Delete product"><i class="fa fa-trash"></i></a>
                 </td>
                </tr>
                <div id="myModal{{ $product->id }}" class="modal hide">
                  ...
                  <div class="modal-body">
                    ...
                    <p><b>Care :</b> <?php echo nl2br($product->care) ?></p>
